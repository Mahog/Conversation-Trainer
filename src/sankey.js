d3.sankey = function() {
  ...
  sankey.sinks = function(_) {
    if (!arguments.length) return sinks;
    sinks = _;
    return sankey;
  }
  ...
  sankey.link = function() {
    ...
    function link(d) {
      ...
      if (typeof d.source.fisheye !== typeof undefined) {
        tx0 = d.source.fisheye.x;
        tx1 = d.target.fisheye.x;
      }

      var x0 = tx0 + d.source.dx,
          x1 = tx1,
  ...
  function computeNodeBreadths() {
    ...
    sinks = d3.range(x).map(function(d) { return []; });
